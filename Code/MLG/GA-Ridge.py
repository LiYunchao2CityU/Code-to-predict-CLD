...
class MyProblem(ga.Problem):
    ...
    def SaveModel(self,A):
        regression = Ridge(alpha=A).fit(self.x_train, self.y_train.ravel())
        dump(regression,"D:/pythonproject/ThesisModel/Linear/result/GA-RidgeModel.joblib")

    def UnseenTest(self):
        model = load("D:/pythonproject/ThesisModel/Linear/result/GA-RidgeModel.joblib")
        ...
        unseen_result.to_excel("D:/pythonproject/ThesisModel/Linear/result/UnseenResultData.xlsx",index=False)
    ...
    def test(self,A):
        ...
        excelFilePath = "D:/pythonproject/ThesisModel/Linear/result/GA-RidgeResultData.xlsx"
...
def subAimFunc(args):
    ...
    y_train = args[3]
    A = Vars[i, 0]
    regression = Ridge(alpha=A).fit(x_train, y_train.ravel())
...
if __name__ == '__main__':
    ...
    if res['success']:
        problem.SaveModel(A=res['Vars'][0, 0])
        problem.UnseenTest()
        problem.test(A=res['Vars'][0, 0])