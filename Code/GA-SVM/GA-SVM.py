...
class MyProblem(ga.Problem):
    ...
    def SaveModel(self,C,G):
        regression = SVR(C=C, kernel='rbf', gamma=G).fit(self.x_train, self.y_train.ravel())
        dump(regression,"D:/pythonproject/ThesisModel/GA-SVM/result/GA-SVRModel.joblib")

    def UnseenTest(self):
        model = load("D:/pythonproject/ThesisModel/GA-SVM/result/GA-SVRModel.joblib")
        ...
        unseen_result.to_excel("D:/pythonproject/ThesisModel/GA-SVM/result/UnseenResultData.xlsx",index=False)
    ...
    def test(self, C, G):
        ...
        excelFilePath = "D:/pythonproject/ThesisModel/GA-SVM/result/GA-SVMResultData.xlsx"
...
def subAimFunc(args):
    ...
    C = Vars[i, 0]
    G = Vars[i, 1]
    regression = SVR(C=C, kernel='rbf', gamma=G).fit(x_train, y_train.ravel())
...
if __name__ == '__main__':
    ...
    if res['success']:
        problem.SaveModel(C=res['Vars'][0, 0], G=res['Vars'][0, 1])
        problem.UnseenTest()
